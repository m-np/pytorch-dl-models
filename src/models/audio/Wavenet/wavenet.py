...
class BERT(nn.Module):
    def __init__(
        ...
        self.embedding = BertEmbedding(
            vocab_size, embed_size=self.dmodel, seq_len=seq_len, pdropout=pdropout
        )
    ...
    def forward(self, token_ids_batch, segment_label):
        ...
        mask = (
            (token_ids_batch > 0)
            .unsqueeze(1)
            .repeat(1, token_ids_batch.size(1), 1)
            .unsqueeze(1)
        )

        # embedding the indexed sequence to sequence of vectors
        token_representations = self.embedding(token_ids_batch, segment_label)
...
class BERTLM(nn.Module):
    ...
    def __init__(
        ...
        self.mlm = MaskedLanguageModel(hidden=self.bert.dmodel, vocab_size=vocab_size)

        self.nsp = NextSentencePrediction(hidden=self.bert.dmodel)
