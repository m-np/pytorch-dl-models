...
class VGG16(nn.Module):
    ...
    def __init__(self, params, device="cpu"):
        ...
        self.fc1 = nn.Sequential(
            ...
            nn.Linear(in_features=7 * 7 * 512, out_features=linear_out),
            nn.ReLU(),
        )

        self.fc2 = nn.Sequential(
            nn.Dropout(dropout_probability),
            nn.Linear(in_features=linear_out, out_features=linear_out),
            nn.ReLU(),
        )

        self.fc3 = nn.Sequential(
            nn.Dropout(dropout_probability),
            nn.Linear(in_features=linear_out, out_features=num_classes),
            nn.ReLU(),
        )
